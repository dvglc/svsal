...
(:~ 
 : Net XQuery-Module
 : This module contains functions for dealing with network requests:
 :  - different types of redirection and forwarding
 :  - content negotiation
 :  - sitemap generation
 :
 ...
 :
 :)
module namespace net                = "http://salamanca.school/ns/net";
...
declare
function net:inject-requestParameter($injectParameter as xs:string*, $injectValue as xs:string*) as xs:string* {
...
declare
function net:redirect-with-301($absolute-path) {
...
declare
function net:redirect-with-307($absolute-path) {
...
(: 303: See other :)
declare
function net:redirect-with-303($absolute-path) {
...
(: 404: does not exist :)
declare
function net:redirect-with-404($absolute-path) {
...
declare
function net:add-parameter($name as xs:string, $value as xs:string) as element(exist:add-parameter) {
    <add-parameter xmlns="http://exist.sourceforge.net/NS/exist" name="{$name}" value="{$value}"/>
};

declare
function net:forward($relative-path as xs:string, $net-vars as map(*)) {
    net:forward($relative-path, $net-vars, ())
};

declare
function net:forward($relative-path as xs:string, $net-vars as map(*), $attribs as element(exist:add-parameter)*) {
...
(: implicit temporary redirect (302) :)
declare
function net:redirect($absolute-path as xs:string, $net-vars as map(*)) {
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
declare
function net:findNode($ctsId as xs:string?){
...
(: This is Golang from https://github.com/golang/gddo/blob/master/httputil/negotiate.go:
...
:)
declare
function net:negotiateContentType($offers as xs:string*, $defaultOffer as xs:string) as xs:string {
...
declare
function local:negotiateCTSub($offers as xs:string*, $bestOffer as xs:string, $bestQ as xs:double, $bestWild as xs:integer) as xs:string {
    let $offer      := $offers[1]
...
(: Sitemap stuff ... :)
declare
function local:getDbDocumentIds($collections as xs:string*) as xs:string* {
...
declare
function local:getDbWebpageRel($documentIds as xs:string*) as xs:string* {
...
declare
function local:getDbWebpageAbs($documentIds as xs:string*, $language as xs:string) as xs:string* {
...
declare
function local:getUrlList($language as xs:string) as element(url)* {
...
declare
function local:localizedSitemap($lang as xs:string) as element(urlset) {
...
declare
function local:sitemapIndex($fileNames as xs:string*) as element(sitemapindex) {
...
declare
function net:sitemapResponse($net-vars as map(*)) {
