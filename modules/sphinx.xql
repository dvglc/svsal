...
(:~ 
 : Sphinx XQuery-Module
 : This module contains the search logic:
 :  - interfacing to sphinxsearch/opensearch server
 :  - UI for search form and results page
 :
 ...
 :
 :)
module namespace sphinx            = "http://salamanca.school/ns/sphinx";
...
(:~
 ...
 :
 : Response format is Open Search based XML
 :  @see http://www.opensearch.org/Home and http://search.salamanca.school/lemmatized
~:)
...
declare
function sphinx:needsSnippets($targetWorkId as xs:string) as xs:boolean {
...
declare
function sphinx:needsSnippetsString($node as node(), $model as map(*)) {
...
declare
function sphinx:passLang($node as node(), $model as map(*), $lang as xs:string?) {
...
declare
function sphinx:buildSelect ($node as node(), $model as map(*), $lang as xs:string?) {
...
declare
function sphinx:keywords ($q as xs:string?) as xs:string {
...
declare
function sphinx:excerpts ($documents as node()*, $words as xs:string) as node()* {
...
declare
function sphinx:highlight ($document as node(), $words as xs:string*) as node()* {
    let $endpoint   := concat($config:sphinxRESTURL, "/excerpts")
...
declare
function sphinx:help ($node as node(), $model as map(*), $lang as xs:string?) {
...
declare
    %public
function sphinx:helpToc($node as node(), $model as map(*)) {
    <div/>
};

declare
    %private
function sphinx:print-sectionsHelp($sections as element()*, $hide as xs:boolean?) {
...
declare
function sphinx:loadSnippets($wid as xs:string*) {
...
declare
function sphinx:addQToCrumbtrail($node as node()*, $q as xs:string*) as item()* {
    typeswitch($node)
...
declare
function local:passthruCrumbtrailQ($nodes as node()*, $q as xs:string*) as item()* {
    for $node in $nodes/node() return sphinx:addQToCrumbtrail($node, $q)
};

declare
function sphinx:dispatch($node as node(), $mode as xs:string) as item()* {
    typeswitch($node)
...
declare
function local:p($node as element(tei:p), $mode as xs:string) as xs:string* {
    for $subnode in $node/node() where (local-name($subnode) ne 'note') return sphinx:dispatch($subnode, $mode)
};

declare
function local:note($node as element(tei:note), $mode as xs:string) as xs:string* {
    local:passthru($node, $mode)
};

declare
function local:div($node as element(tei:div), $mode as xs:string) as xs:string* {
    if ($mode = "edit") then
...
declare
function local:milestone($node as element(tei:milestone), $mode as xs:string) as xs:string* {
    ()
  (:    if ($mode = "edit") then
        if ($node/@n and not(matches($node/@n, '^[0-9]+$'))) then
...
declare
function local:abbr($node as element(tei:abbr), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
...
declare
function local:orig($node as element(tei:orig), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
...
declare
function local:sic($node as element(tei:sic), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
...
declare
function local:expan($node as element(tei:expan), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then ()
    else
        local:passthru($node, $mode)
};
...
declare
function local:reg($node as element(tei:reg), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then ()
    else
        local:passthru($node, $mode)
};
...
declare
function local:corr($node as element(tei:corr), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then ()
    else
        local:passthru($node, $mode)
};
...
declare
function local:g($node as element(tei:g), $mode as xs:string) as xs:string* {
    if ($mode="orig") then
...
declare
function local:persName($node as element(tei:persName), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
...
declare
function local:placeName($node as element(tei:placeName), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
        if ($node/@key) then
...
declare
function local:term($node as element(tei:term), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
        if ($node/@key) then
...
declare
function local:title($node as element(tei:title), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
        if ($node/@key) then
...
declare
function local:bibl($node as element(tei:bibl), $mode as xs:string) as xs:string* {
    if ($mode = "orig") then
        local:passthru($node, $mode)
    else
